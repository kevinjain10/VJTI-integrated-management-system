...
$query="SELECT * FROM `groundentry` WHERE `username`='{$_SESSION['user_name']}'";
 ...
 {    switch($row["slotno"])
      { case 1:$st='7:00am-10:00am';break;
        case 2:$st='10:00am-01:00pm';break;
        case 3:$st='02:00pm-04:00pm';break;
        case 4:$st='05:00pm-07:00pm';break;
        case 5:$st='08:00pm-11:00pm';break;
        default:echo 'invalid input';

      }
      $_SESSION['groundid']=$row["id"];
   ...
   {  $id=$_POST['id'];
      $sql="SELECT * FROM `groundentry` WHERE `username`='{$_SESSION['user_name']}' AND `id`='$id'";
      ...
      {  if(mysqli_num_rows($sql_run)==0)
         ...
         if(mysqli_num_rows($sql_run)==1)
           { $querry="DELETE FROM `groundentry` WHERE `id`='$id'";
             if($querry_run=mysqli_query($conn,$querry))
             header('Location:ground-delete.php');
             else
   ...
   <form action="ground-delete.php" method="POST">
   <br><br>
...
<div id="section">
 To go to ground-homepage
<button onclick="location.href='ground-homepage.php'" class="btn btn-primary">Home</button>

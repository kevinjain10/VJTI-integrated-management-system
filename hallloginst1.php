...
<nav class="navbar navbar-inverse" style="height: 60px; font-size: 20px">
  <div class="container-fluid" style="padding-bottom:20px; padding-top:5px">
    <div class="navbar-header">
      ...
      <a class="navbar-brand" href="staff-homepage.php" style="margin-left: 5px; font-size: 20px">INTEGRATED MANAGEMENT SYSTEM VJTI</a>
    </div>
        ...
        if(mysqli_query($conn,$queri))
         {
          header('Location:staff-homepage.php');
              ...
              {     $row = mysqli_fetch_assoc($query_run);
                ...
                {  $queri="UPDATE `hallentry` SET `id`='{$row["id"]}',`slotno`='{$_SESSION['slotno']}',`date`='{$_SESSION['date']}',`username`='{$_SESSION['user_name']}',`purpose`='{$_SESSION['purpose']}',`halltype`='{$_SESSION['type']}',`usertype`='{$_SESSION['usertype']}',`status`='view' WHERE `id`='{$row["id"]}'";
                   if(mysqli_query($conn,$queri))
                {
          header('Location:staff-homepage.php');
         	}
...
<form action="hallloginst1.php" method="POST">
...
<tr>
  ...
  <td> <select name="halltype" style="width: 250px;">

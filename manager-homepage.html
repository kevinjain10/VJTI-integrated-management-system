...
$query="SELECT * FROM `hallentry` WHERE `status`='view'";
 ...
 { if(mysqli_num_rows($query_run)==0)
   ...
   ?>
     <div class="col-md-12 text-center">
	  <br>
	    <button class="btn btn-success btn-lg" type="button" onclick="window.location.href='manager-insert.php'">BOOK</button>
	  </div>

	  <?php
   }
   if(mysqli_num_rows($query_run)>=1)
   {echo 'Current bookings are:<br><br>';
 ...
 {    switch($row["slotno"])
   ...
   ?>
	    ...
	    <button class="btn btn-primary btn-lg" type="button" onclick="window.location.href='manager-update.php'">UPDATE</button>
	  </div>
   <?php
  ...
  if(isset($_POST['deletesr'])&&!empty($_POST['deletesr']))
   {
     $id=$_POST['deletesr'];
      ...
      $sql="SELECT * FROM `hallentry` WHERE `id`='$id'";

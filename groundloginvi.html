...
<nav class="navbar navbar-inverse" style="height: 60px; font-size: 20px">
  <div class="container-fluid" style="padding-bottom:20px; padding-top:5px">
    <div class="navbar-header">
      ...
      <a class="navbar-brand" href="visitor-homepage.php" style="margin-left: 5px; font-size: 20px">INTEGRATED MANAGEMENT SYSTEM VJTI</a>
    </div>
       ...
       if(mysqli_num_rows($query_run)==0)
             {
               ...
               $queri="INSERT INTO `groundentry`(`id`,`slotno`,`date`,`username`,`purpose`,`groundtype`,`usertype`,`status`)
     VALUES('','{$_SESSION['slotno']}','{$_SESSION['date']}','{$_SESSION['user_name']}','{$_SESSION['purpose']}','{$_SESSION['type']}','{$_SESSION['usertype']}','view')";
        if(mysqli_query($conn,$queri))
         {
          header('Location:visitor-homepage.php');
              ...
              {     $row = mysqli_fetch_assoc($query_run);
                ...
                {  $queri="UPDATE `groundentry` SET `id`='{$row["id"]}',`slotno`='{$_SESSION['slotno']}',`date`='{$_SESSION['date']}',`username`='{$_SESSION['user_name']}',`purpose`='{$_SESSION['purpose']}',`groundtype`='{$_SESSION['type']}',`usertype`='{$_SESSION['usertype']}',`status`='view' WHERE `id`='{$row["id"]}'";
                   if(mysqli_query($conn,$queri))
                {
          header('Location:visitor-homepage.php');
         	}
...
<form action="groundloginvi.php" method="POST">
...
<tr>
  ...
  <td>Time slot:</td>
  <td> <select name="time" style="width: 250px;">
...
<tr>
  <td>Purpose:</td>
  <td><textarea name="purpose" rows="2" cols="10" style="width: 250px;"></textarea></td> <br><br>
  <td>Type of the ground:</td>
  <td> <select name="groundtype" style="width: 250px;">
